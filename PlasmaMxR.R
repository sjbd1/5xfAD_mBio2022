...
######## Correlation between Most Abundant Plasma Metabolites & Selected Microbes ################
...
# Use grep to select for subsets (e.g., 18 mo FECAL or CECAL)
otus <- otus[grep("FECAL", otus$Type),]
otus <- otus[grep("18mo", otus$Cohort),]

rownames(otus) <- otus$Mouse_ID
...
metab_meta <- as.data.frame(read.table("./PlasmaMX_data.txt",
                                       fill = TRUE,
                                       row.names = 1,
                                       quote=NULL, 
                                       sep="\t", 
...
#Set NA values to 0
metab_meta[is.na(metab_meta)] <- 0

# create a metadata table that contains only the samples present in both metabolite & microbe data
select_otus <- rownames_to_column(select_otus, var = "Mouse_ID")
merged_data <- merge(metab_meta, select_otus,
                     by.x="Mouse_ID", by.y="Mouse_ID", 
                     all.x = FALSE, all.y = FALSE, sort = TRUE)
merged_data <- column_to_rownames(merged_data, var = "Mouse_ID")

#Select for metabolite data only
mx <- merged_data[,(10:(ncol(merged_data)-17))]
...
#Cecal Results:
  #Mantel statistic r: -0.009704 
  #Significance: 0.5174 
#Fecal Results:
  #Mantel statistic r: -0.07014 
  #Significance: 0.7234 
